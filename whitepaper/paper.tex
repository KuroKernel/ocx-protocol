...
\midrule
\multicolumn{5}{l}{\textit{Cross-vendor: AMD Instinct MI300X (CDNA3 gfx942), ROCm 6.1, PyTorch 2.5.1+rocm6.1}} \\
...
Qwen 2.5-72B-Instruct & 2$\times$MI300X tensor-parallel (\texttt{tp\_plan=auto}, RCCL fabric), bf16 & 58 tokens & 3 & byte-identical \emph{within MI300X; differs from H100 TP as expected} \\
\bottomrule
\end{tabular}
\caption{Byte-identical inference across fresh \texttt{torchrun} launches. First eight rows: 22 launches across 8 model$\times$configuration groups on NVIDIA Hopper, every group internally byte-identical. Last four rows: 12 launches across 4 model$\times$configuration groups on AMD CDNA3, every group internally byte-identical. The three single-GPU MI300X rows match the corresponding NVIDIA H100 single-GPU CPU-offload baseline byte-for-byte across vendors. The 2$\times$MI300X tensor-parallel row deliberately uses RCCL fabric all-reduce instead of NCCL ring all-reduce; the resulting hash differs from the H100 2$\times$TP hash, drawing the empirical boundary of cross-vendor byte-identity at the multi-GPU collective layer (\S\ref{sec:empirical}).}
...
\paragraph{Where cross-vendor byte-identity ends.} We then ran the same poem prompt on \textbf{2$\times$MI300X with HuggingFace Transformers' native tensor parallelism (\texttt{tp\_plan="auto"}, dispatching collectives through RCCL fabric all-reduce)}. Three fresh \texttt{torchrun} launches each produced \texttt{output\_hash dd3fed4a55f1a550ebc16efe9ec84452f9cec5f67e89ee688030735316fff936} (58 tokens emitted; both ranks always agreeing per the all-gather check inside the script). The within-vendor byte-identity holds. The cross-vendor byte-identity does \emph{not}: the AMD 2$\times$TP hash differs from the NVIDIA H100 2$\times$TP baseline (\texttt{f8dc73cb...}, 54 tokens), and it also differs from the AMD 1-GPU baseline (\texttt{e59fca7d...}, 51 tokens). Different reduction trees produce different intermediate logits, which under greedy decoding produce different argmax tokens, which compound into different texts. Both are individually deterministic; cross-vendor equality at the multi-GPU collective layer is not.

This is exactly the boundary the underlying physics predicts. The cuBLAS and hipBLAS GEMM kernels happen to coincide functionally for the shapes we use, so single-GPU reductions agree; NCCL ring topology over NVLink and RCCL fabric topology over AMD Infinity Fabric do not coincide, so the collective reductions disagree. This is empirical evidence, not theorem: at single-GPU scale, the receipt's \texttt{output\_hash} is portable across NVIDIA Hopper and AMD CDNA3; at multi-GPU scale, it is not. The receipt's environment binding (parallelism configuration, vendor SKU, driver, runtime) makes the configuration explicit so a verifier knows which substrate to re-execute on.

The small-model cross-architecture result on Blackwell vs Hopper above similarly cannot be assumed to generalise: ARM64 vs x86\_64 CPU floating-point libraries diverge at LLM scale (we measured this in earlier work). The cross-vendor positive result for single-GPU compute is a useful empirical surprise; the cross-vendor negative result for multi-GPU collectives is a confirmation of the underlying topology argument.
...
Cross-architecture and cross-vendor inference \emph{can} produce different output hashes for the same weights and inputs, by the physics of floating-point reduction order. The receipt's environment binding makes the configuration explicit, so a verifier knows which substrate to re-execute on. \S\ref{sec:empirical} documents both sides of this boundary empirically: at single-GPU bf16 with eager attention, AMD MI300X (CDNA3) and NVIDIA H100 (Hopper) produce \emph{byte-identical} output hashes for Qwen 2.5-72B over 51 tokens of compounding reductions; at 2$\times$ tensor-parallel scale the same model produces \emph{different} hashes across vendors, because NCCL ring all-reduce and RCCL fabric all-reduce reduce in different orders. Both are individually deterministic. We have not yet measured Intel Gaudi or Google TPU.

We have measured vLLM 0.19.1 on NVIDIA H100 with Qwen 2.5-7B, greedy sampling, bf16, seed=42: byte-deterministic across three cold-started Python processes and across five identical requests within each process (1 unique \texttt{output\_hash} in each cell). The vLLM hash does not match the HF Transformers eager-attention reference path on the same hardware (different fused-kernel reductions), but both stacks are individually byte-deterministic. The earlier received-wisdom that vLLM's PagedAttention is too non-deterministic to wrap was wrong for this configuration. We have not yet measured vLLM under sampling, concurrent dynamic batching, prefix caching across prompts, or on AMD ROCm; and we have not measured TensorRT-LLM or exllamav2.
