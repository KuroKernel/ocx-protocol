...
type VMConfig struct {
	...
	// StrictMode enables strict enforcement of execution rules
	// When true: non-zero exit codes and determinism violations cause execution to fail
	// When false: warnings are logged but execution continues (permissive mode)
	StrictMode bool
...
type ExecutionResult struct {
	...
	// GasUsed is the deterministic gas consumed (for receipts)
	GasUsed uint64

	// HostCycles tracks host-measured computational cycles (for diagnostics)
	HostCycles uint64
	...
	// HostInfo contains variable host system information
	HostInfo HostInfo
}
...
func DefaultVMConfig() VMConfig {
	return VMConfig{
		...
		CycleLimit:  100_000_000,      // Increased for shell script execution
...
// Implements Option 1: Deterministic gas model
type OCXReceipt struct {
	// Signed Core (deterministic - same artifact+input = same values)
	...
	GasUsed      uint64   `cbor:"5,keyasint"` // Deterministic gas (was GasUsed)
	IssuerID     string   `cbor:"6,keyasint"`
	Signature    []byte   `cbor:"7,keyasint"`

	// Unsigned Metadata (variable - for diagnostics/audit only)
	HostCycles uint64   `cbor:"8,keyasint"`  // Host-measured cycles
	StartedAt  uint64   `cbor:"9,keyasint"`  // Wall-clock start time
	FinishedAt uint64   `cbor:"10,keyasint"` // Wall-clock end time
	DurationMs uint64   `cbor:"11,keyasint"` // Wall-clock duration
	MemoryUsed uint64   `cbor:"12,keyasint"` // Peak memory usage
	HostInfo   HostInfo `cbor:"13,keyasint"` // Host system info
}

// HostInfo contains variable host system information (not signed)
type HostInfo struct {
	CPUModel         string   `cbor:"1,keyasint"`
	KernelVer        string   `cbor:"2,keyasint"`
	Platform         string   `cbor:"3,keyasint"`
	LoadAvg          string   `cbor:"4,keyasint"`
	MonitoringMethod string   `cbor:"5,keyasint"`
	MemoryMethod     string   `cbor:"6,keyasint"`
	IsolationLevel   string   `cbor:"7,keyasint"`
	ActiveNamespaces []string `cbor:"8,keyasint"`
	CgroupPath       string   `cbor:"9,keyasint"`
}

// SignedCore represents only the deterministic fields that are signed
type SignedCore struct {
	SpecHash     [32]byte `cbor:"1,keyasint"`
	ArtifactHash [32]byte `cbor:"2,keyasint"`
	InputHash    [32]byte `cbor:"3,keyasint"`
	OutputHash   [32]byte `cbor:"4,keyasint"`
	GasUsed      uint64   `cbor:"5,keyasint"`
	IssuerID     string   `cbor:"6,keyasint"`
	Signature    []byte   `cbor:"7,keyasint"`
}

// GetSignedCore extracts only the deterministic signed fields from a receipt
func (r *OCXReceipt) GetSignedCore() *SignedCore {
	return &SignedCore{
		SpecHash:     r.SpecHash,
...
// Only the signed core is canonicalized for deterministic receipts
func CanonicalizeReceipt(receipt *OCXReceipt) ([]byte, error) {
	// Get only the signed core for canonicalization
	signedCore := receipt.GetSignedCore()

	// Encode to canonical CBOR
	canonical, err := CanonicalCBORMode.Marshal(signedCore)
	if err != nil {
		return nil, fmt.Errorf("canonical encoding failed: %w", err)
	}

	// Verify idempotence: decode and re-encode should be identical
	var decoded SignedCore
...
// =============================================================================
// DETERMINISTIC RNG UTILITIES (Legacy - use deterministic_rng.go instead)
// =============================================================================
